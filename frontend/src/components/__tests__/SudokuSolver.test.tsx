...
describe('SudokuSolver', () => {
  ...
  describe('Mobile mode detection', () => {
    ...
    it('enables mobile mode when touch device and narrow screen', () => {
      // Set narrow screen
      Object.defineProperty(window, 'innerWidth', {
        writable: true,
        configurable: true,
      ...
      (window as unknown as { ontouchstart: () => void }).ontouchstart = () => {}

      render(<SudokuSolver />)

      // In mobile mode, NumberPad should be shown
      // We check by looking for the hint text that NumberPad displays
      expect(screen.getByText('セルをタップして選択してください')).toBeInTheDocument()
    })

    it('does not enable mobile mode on wide screen', () => {
      // Set wide screen
      Object.defineProperty(window, 'innerWidth', {
        writable: true,
        configurable: true,
      ...
      // NumberPad should not be shown
      expect(screen.queryByText('セルをタップして選択してください')).not.toBeInTheDocument()
    })

    it('does not enable mobile mode on non-touch device', () => {
      // Set narrow screen
      Object.defineProperty(window, 'innerWidth', {
        writable: true,
        configurable: true,
