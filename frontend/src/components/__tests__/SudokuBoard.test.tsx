...
describe('SudokuBoard', () => {
  ...
  describe('Mobile Mode', () => {
    it('renders tappable divs instead of inputs in mobile mode', () => {
      const mockOnChange = jest.fn()
      ...
      // Should not have textbox inputs
      expect(screen.queryAllByRole('textbox')).toHaveLength(0)
      // Should have button elements
      expect(screen.getAllByRole('button')).toHaveLength(81)
    })

    it('calls onCellSelect when cell is clicked in mobile mode', () => {
      const mockOnChange = jest.fn()
      const mockOnCellSelect = jest.fn()
    ...
    it('calls onCellSelect when Enter key is pressed on cell', () => {
      const mockOnChange = jest.fn()
      const mockOnCellSelect = jest.fn()
    ...
    it('calls onCellSelect when Space key is pressed on cell', () => {
      const mockOnChange = jest.fn()
      const mockOnCellSelect = jest.fn()
    ...
    it('highlights selected cell with stronger blue in mobile mode', () => {
      const mockOnChange = jest.fn()
      const mockOnCellSelect = jest.fn()
      ...
      const cells = screen.getAllByRole('button')
      const selectedCell = cells[0] as HTMLElement

      expect(selectedCell.style.backgroundColor).toBe('rgb(179, 217, 255)')
      expect(selectedCell.style.boxShadow).toBe('inset 0 0 0 3px #007bff')
    })

    it('has correct aria-label for cells', () => {
      const boardWithValue = emptyBoard.map((row, rowIndex) =>
        row.map((cell, colIndex) =>
          rowIndex === 0 && colIndex === 0 ? 5 : cell
        )
